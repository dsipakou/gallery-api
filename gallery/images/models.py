...
class Image(models.Model):
    uuid = models.CharField(
        _("image uuid"), max_length=36, null=True, blank=True, unique=True
    )
    ...
    photo_preview = ImageSpecField(
        source="photo", processors=[ResizeToCover(400, 400)], format="PNG"
    )
    ...
    def admin_image_tag(self):
        return mark_safe(
            '<img style="width: 150px;" src="{}" />'.format(self.photo_preview.url)
        )
